...
#define MAVLINK_MSG_ID_KEY_COMMAND_CRC 203
#define MAVLINK_MSG_ID_229_CRC 203
...
static inline uint16_t mavlink_msg_key_command_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                               uint64_t time_usec, char command)
{
...
{
    return mavlink_msg_key_command_pack(system_id, component_id, msg, key_command->time_usec, key_command->command);
}
...
{
    return mavlink_msg_key_command_pack_chan(system_id, component_id, chan, msg, key_command->time_usec, key_command->command);
}
...
static inline void mavlink_msg_key_command_send(mavlink_channel_t chan, uint64_t time_usec, char command)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_KEY_COMMAND_LEN];
    _mav_put_uint64_t(buf, 0, time_usec);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_key_command_send(chan, key_command->time_usec, key_command->command);
#else
 ...
 */
static inline void mavlink_msg_key_command_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint64_t time_usec, char command)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
